...
```
    public class Main {
        public static void main(String[] args) {
            ...
            boolean person = list.add("eye");   这里的返回值代表是否添加成功,不应该被命名为person,变量的名字一定与含义或用途对应,才能保证以后能看懂自己写的代码,也方便其他人看.
...
> 这个和我想让你做的不太符合,你需要做以下几件事.

> 1.新建一个Person内部类,类中有三个私有属性,分别是ID,名字和年龄.

> 2.创建若干个Person类型对象,并初始化他们的私有属性.

> 3.将这些Person对象添加到一个新建的ArrayList中.

> 4.删除ID为3的Person对象.

> 5.遍历这个ArrayList,以ID:0,名字:jack,年龄:20,这样的格式输出结果.
...
>这些ArrayList之类的东西,其实就是别人写好了的代码,你可以直接拿来用(类似于你想拧螺丝,你可以直接拿别人做好的扳手用,不需要自己再去做一个扳手),点击进去就是看看人家当初是怎么设计这个工具的.
...
> 栈和队列是两个非常重要,也非常基础的概念.一个是FIFO(fisrt in first out),一个是FILO(first in last out),这个区别记住即可.
操作栈的方法

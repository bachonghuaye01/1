...
![A2R)H_%$%{A~YKD4)`67A8T](https://user-images.githubusercontent.com/110222247/231464537-88a05e43-f62b-434e-a80f-86420e3c754c.png)
...
LinkedList 底层是用双向链表实现的存储。链表分三种，单向链表，双向链表，双向循化链表

链表的特点：查询效率低，增删效率高，线程不安全。

**双向链表**也叫双链表，是链表的一种，它的每个数据节点中都有两个指针，分别指向前一个节点和后一个节点，所以从双向链表中的任意一个节点开始，都可以方便的找到所有节点。

可以将节点理解成火车车厢，从任意车厢上车都可以去到不论前后的任何一个车厢


**LinkedList 的使用（List标准）
...
> 这只是Hash算法的一个最简单的例子,实际HashSet或HashMap不适用这个算法.
...
TreeSet 是一个可以对元素进行排序的容器。他的底层实际是用 TreeMap 实现的，内部维持了一个简化版的 TreeMap，通过 Key 来存储 Set 的元素。TreeSet 内部需要对存储的元素进行排序，因此，我们需要给定排序规则。
...
> 试试Ctrl+O或者Alt+Insert,应该是重写的快捷键.
...
public class HashMapTest {
    public static void main(String[] args){
        ...
        //因为 a 在容器中已经存在，所以 B 会把 A 覆盖掉，同时会把 A 返回
        
        System.out.println(value);
        
        //遇到相同 put 的元素时，新的 value 会把 旧的 value 覆盖掉
        //他的返回值在有覆盖的情况下，会把被覆盖的返回，否则为空（null）
